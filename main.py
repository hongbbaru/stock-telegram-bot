...
def get_soxx_report():
    ...
    # 상단: 5년 차트
    axes[0].plot(df.index, df['FG'], color='#1f77b4', linewidth=1.2)
    axes[0].axhline(75, color='r', ls='--', alpha=0.5); axes[0].axhline(25, color='g', ls='--', alpha=0.5)
    axes[0].set_title('SOXX Fear & Greed Index (5-Year Long-term)', fontweight='bold')
    axes[0].set_ylim(0, 100); axes[0].grid(True, alpha=0.3)
    
    # 하단: 1년 차트
    df_1y = df.tail(252)
    axes[1].plot(df_1y.index, df_1y['FG'], color='#1f77b4', linewidth=1.8)
    axes[1].axhline(75, color='r', ls='--', alpha=0.5); axes[1].axhline(25, color='g', ls='--', alpha=0.5)
    axes[1].set_title('SOXX Fear & Greed Index (1-Year Recent)', fontweight='bold')
...
def get_kospi_report():
    ...
    # 상단: 5년 차트
    axes[0].plot(df.index, df['FG'], color='#d62728', linewidth=1.2)
    axes[0].axhline(75, color='r', ls='--', alpha=0.5); axes[0].axhline(25, color='g', ls='--', alpha=0.5)
    axes[0].set_title('KOSPI Fear & Greed Index (5-Year Long-term)', fontweight='bold')
    axes[0].set_ylim(0, 100); axes[0].grid(True, alpha=0.3)
    
    # 하단: 1년 차트
    df_1y = df.tail(252)
    axes[1].plot(df_1y.index, df_1y['FG'], color='#d62728', linewidth=1.8)
    axes[1].axhline(75, color='r', ls='--', alpha=0.5); axes[1].axhline(25, color='g', ls='--', alpha=0.5)
    axes[1].set_title('KOSPI Fear & Greed Index (1-Year Recent)', fontweight='bold')
    axes[1].set_ylim(0, 100); axes[1].grid(True, alpha=0.3)
...
def send_all_reports():
    ...
    requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendPhoto", data={'chat_id': CHAT_ID}, files={'photo': s_img})
    ...
    requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendPhoto", data={'chat_id': CHAT_ID}, files={'photo': k_img})
    
    print("🎉 5년 & 1년 통합 차트 리포트 발송 완료!")
